...
def calculate_and_save_stats(dataset, stats_path):
    ...
    try:
        if not hasattr(dataset, 'dataset') or not hasattr(dataset.dataset, 'processed_data'): 
             raise ValueError("Dataset structure incorrect. Expected Wrapper -> DogPoseDataset -> processed_data")
        ...
        if not all_data:
            ...
            try: c_in = dataset.dataset.processed_data[0].shape[2] if len(dataset.dataset.processed_data) > 0 else 3
            except: c_in = 3
...
class DogPoseDataset(Dataset):
    ...
    def __init__(self, json_paths, labels, seg_len, stats_path=None, train=True):
        ...
        # 1.5. [핵심 수정] 스무딩 + 스케일링 전처리를 __init__에서 1회만 수행
        print(f"Applying pre-processing (smoothing/interpolation/scaling) to {len(raw_data_list)} videos...")
        self.processed_data = [] # 전처리가 완료된 데이터
        for video_array in tqdm(raw_data_list, desc="Pre-processing videos"):
            smoothed_video = self._smooth_and_interpolate(video_array)
            # 스무딩된 비디오를 스케일링
            normalized_scaled_video = self._normalize_and_scale(smoothed_video)
            self.processed_data.append(normalized_scaled_video)
        print("Pre-processing complete.")
        del raw_data_list # 메모리 확보

        # 1.6. 통계 로드 (이제 processed_data 기준으로)
        ...
        # 2. 슬라이딩 윈도우 샘플 생성
        self.samples = []
        stride = 1 if self.train else 5 
        
        for video_idx in range(len(self.processed_data)):
            video_data = self.processed_data[video_idx] 
    ...
    def __getitem__(self, idx):
        ...
        # 2. 샘플링 / 패딩
        if start_frame == -1:
            sample = self._pad_zero(sample_processed, self.seg_len)
        else:
        ...
        # 3. [삭제] _normalize_and_scale 호출 제거 (이미 __init__에서 완료)
        # sample = self._normalize_and_scale(sample)

        # 4. 증강 (Augmentation) - 이것만 훈련 시 실시간으로 적용
        if self.train:
    ...
    def _load_data(self, json_paths):
        ALL_KEYPOINT_NAMES = ["left_f_wrist", "left_f_ankle", "left_f_shoulder", "left_b_wrist", "left_b_ankle", "left_b_shoulder", "right_f_wrist", "right_f_ankle", "right_f_shoulder", "right_b_wrist", "right_b_ankle", "right_b_shoulder", "tail_s", "tail_e", "left_mid_ear", "right_mid_ear", "nose", "mouth", "left_edge_ear", "right_edge_ear"]
