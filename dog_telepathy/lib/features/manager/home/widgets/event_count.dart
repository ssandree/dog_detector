...
class EventStats {
  static int recentEventCount(List<EventInfo> events,
  ...
  static int patellaAlertCount(List<EventInfo> events) {
    return events
        .where((event) =>
            (event.patellaAnalysisResult ?? '').trim().isNotEmpty)
        .length;
  }
}
...
class EventCountCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final request = PetEventsRequest(
      petId: petId,
      skip: 0,
      limit: 50,
    );
    final eventsAsync = ref.watch(petEventsProvider(request));
