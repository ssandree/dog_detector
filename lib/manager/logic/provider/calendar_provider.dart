...
class CalendarNotifier extends Notifier<CalendarState> {
  ...
  CalendarState build() {
    ...
    ref.listen<pet_domain.PetInfo?>(currentPetProvider, (previous, next) {
  ...
  void changeMonth(DateTime date) {
    ...
    if (state.petId == 0) return; // petId가 0이면 변경하지 않음
    state = state.copyWith(
      year: date.year,
      month: date.month,
    );
