...
import 'calendar_data_calculator.dart';
...
class _CalendarSectionState extends ConsumerState<CalendarSection> {
  ...
  Widget build(BuildContext context) {
    // 주간 리포트에서 선택된 주간 감지 및 변경 리스닝
    ref.watch(weeklyReportDateProvider);
    
    // 주간이 변경되었을 때 하이라이트
    ref.listen<DateTime>(weeklyReportDateProvider, (previous, next) {
      final nextOnly = DateTime(next.year, next.month, next.day);
      if (previous == null || !DateTime(previous.year, previous.month, previous.day).isAtSameMomentAs(nextOnly)) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          _highlightWeek(nextOnly);
        });
      }
    });

    // 날짜별 감정 비율 및 상태 계산 (mockDailyReports 기반)
    final weeklyRatios = CalendarDataCalculator.calculateEmotionRatios(
      focusedDay: _focusedDay,
      mockDailyReports: mockDailyReports,
    );
    return Container(
